...
async def build_gdpr_export_package(db: AsyncSession, user_id: str) -> bytes:
    ...
    exp_csv = ["id,category,total_amount,status,merchant,created_at"]
    for e in expenses:
        exp_csv.append(f"{e.id},{e.category},{e.total_amount},{e.status},{e.merchant},{e.created_at}")
    ...
    k_csv = ["id,sender_id,badge,message,created_at"]
    for k in kudos_list:
        k_csv.append(f"{k.id},{k.sender_id},{k.badge},{k.message},{k.created_at}")
    sections["kudos_received.csv"] = "\n".join(k_csv)

    try:
        course_res = await db.execute(
            select(CourseEnrollment).where(CourseEnrollment.user_id == user_id).order_by(CourseEnrollment.created_at.desc()).limit(50)
        )
        courses = course_res.scalars().all()
        c_csv = ["id,course_id,status,enrolled_at,completed_at"]
        for c in courses:
            c_csv.append(f"{c.id},{c.course_id},{c.status},{c.created_at},{c.completed_at}")
    ...
    try:
        ...
        v_csv = ["id,start_date,end_date,absence_type,status"]
        for v in vacs:
            v_csv.append(f"{v.id},{v.start_date},{v.end_date},{v.absence_type or 'vacation'},{v.status}")
